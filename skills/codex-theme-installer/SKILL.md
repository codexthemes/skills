...
Report the installed path (`~/.codexthemes/themes/<theme-id>/`), the theme version, and the files written. Installation places source files only — it does not change the running Codex app. To see the theme in Codex, hand off to codex-theme-creator's reversible apply workflow (`apply-theme.ts apply <theme-dir>`), which asks the user before restarting the app. Never claim a theme is active in Codex just because the files were installed.
